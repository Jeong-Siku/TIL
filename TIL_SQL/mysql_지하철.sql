...
CREATE TABLE TB_PBTRNSP (
  ...
  NMPR_CNT BIGINT, -- 인원수
  PRIMARY KEY (STATN_NO, STD_MT, BEGIN_TIME, END_TIME, TKCAR_GFF_SE_CD) # 다섯개의 값이 모두 중복되지않는한 데이터가 들어간다. 카테고리 데이터가 많을 시 사용, 
);
...
ORDER BY CAST(STATN_NO AS UNSIGNED), BEGIN_TIME, END_TIME; # CAST 타입변경
...
# 송하차 구분 코드 별로 인원수가 많은 순으로 조회. 역 번호, 역 이름, 호선, 승하차 여부, 승하차 별 합
SELECT  A.STATN_NO,
		A.STATN_NM,
        A.HO_LN,
        A.TKCAR_GFF_SE_CD,
...
GROUP BY A.STATN_NO,A.STATN_NM,A.HO_LN,A.TKCAR_GFF_SE_CD # 집합레벨 변화를 위해 그룹에 여러개를 묶음	
...
FROM (
	...
	FROM (	
		...
		GROUP BY A.STATN_NO, A.STATN_NM,A.HO_LN) A
        ) B
...
FROM TB_PBTRNSP A
WHERE A.STD_MT = '202304'
  AND A.BEGIN_TIME = '0800' AND A.END_TIME = '0900'
...
WHERE A.STD_MT = '202304'
  ...
  AND A.TKCAR_GFF_SE_CD = 'GF'
GROUP BY A.STATN_NO, A.STATN_NM, A.HO_LN # 프라이머리 키들은 동시에 중복되지 않도록 하나의 키값으로 생각하고 묶어 기준으로 만든다. (PK1, PK2, PK3) = KEY
ORDER BY NMPR_CNT DESC                   # 집합관계! GROUP BY에 쓰는 컬럼의 순서도 중요! 순서에 따라 데이터들의 관계가 달라진다.
LIMIT 10;
...
SELECT * FROM TB_PBTRNSP;
# 각 호선별 승하차 수가 가장 많은 역
SELECT A.STATN_NO, A.STATN_NM,A.HO_LN, A.NMPR_CNT,A.RANk_NMPR_CNT
FROM (    
    SELECT A.STATN_NO, A.STATN_NM,A.HO_LN, A.NMPR_CNT,
		  ROW_NUMBER() OVER(PARTITION BY A.HO_LN ORDER BY A.NMPR_CNT DESC) as RANk_NMPR_CNT
	FROM (
		...
		WHERE A.STD_MT = '202304'
		GROUP BY A.STATN_NO, A.STATN_NM, A.HO_LN
		ORDER BY NMPR_CNT DESC) A
        ) A
WHERE A.RANk_NMPR_CNT=1
